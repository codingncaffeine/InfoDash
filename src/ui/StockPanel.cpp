...
void StockPanel::updateStocks(const std::vector<StockData>& data) {
    ...
    for (const auto& s : data) {
        ...
        GtkWidget* symLabel = gtk_label_new(s.symbol.c_str());
        gtk_widget_add_css_class(symLabel, "article-title");
        gtk_box_append(GTK_BOX(header), symLabel);

        GtkWidget* nameLabel = gtk_label_new(s.name.c_str());
        gtk_widget_add_css_class(nameLabel, "article-source");
